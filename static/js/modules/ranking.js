...
function renderAllRanking(data) {
    ...
    contentDiv.innerHTML = Object.values(data)
        .map(disciplineData => rankingCard(disciplineData, 'mb-8 print-break'))
        .join('');
}
...
function renderRanking(data) {
    ...
    if (data.kind !== 'team' && (!data.rankings || data.rankings.length === 0)) {
        contentDiv.innerHTML = `
            <div class="text-center py-12">
                <svg class="h-12 w-12 mx-auto mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"></path>
    ...
    contentDiv.innerHTML = rankingCard(data);
}

// Team disciplines get the team table, all others the individual one
function rankingCard(data, extraClass = '') {
    return data.kind === 'team' ? teamRankingCard(data, { extraClass }) : individualRankingCard(data, extraClass);
}

function individualRankingCard(data, extraClass) {
    return `
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider" title="Distance of the furthest shot; the lower value wins">Tie-break</th>
